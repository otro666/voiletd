...
void pump() {
    ...
    if (!g_linked && g_haveRoom) {
        if (net::neighbourCount() > 0) {
            // Предложение кладём раз в 10 с, комнату опрашиваем раз в 2 с: телефон
            // отвечает не мгновенно — ему нужно собрать визитку и опросить комнату.
            if (now - g_lastOffer > 10000) { g_lastOffer = now; publishOffer(); }
            if (now - g_lastPoll > 2000)   { g_lastPoll = now;  pollRoom(); }
        } else {
            // Причина простоя — В ЖУРНАЛ, раз в полминуты. Прошлый раз условие молча
            // не выполнялось 7 минут, и по журналу было не понять, чего именно ждём:
            // комнату, соседей или чуда. Телефон появляется в соседях, когда его
            // опросник постучится к нам — для этого оба должны быть в одной сети.
            static uint32_t lastWhy = 0;
            if (now - lastWhy > 30000) {
                lastWhy = now;
                store::log("phone", "комната задана, соседей в сети нет — жду телефон");
            }
        }
    }
