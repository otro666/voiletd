...
void pump() {
    ...
    if (!g_linked && g_haveRoom) {
        // Рассылаем ПО РЕЛЬСАМ БЕЗУСЛОВНО. Прежнее условие «есть соседи в локальной
        // сети» осталось здесь от первой версии, когда встреча шла только через DHT, —
        // и обесценивало всю работу по рельсам: они были готовы, а вызывать их было
        // некому. Соседи по локальной сети нужны только для DHT-пути, и решение об
        // этом принимается внутри рассылки, а не здесь.
        if (now - g_lastOffer > 10000) { g_lastOffer = now; publishOffer(); }
        if (now - g_lastPoll > 2000 && net::neighbourCount() > 0) {
            g_lastPoll = now;
            pollRoom();          // опрос комнаты — только у соседей по локальной сети
        }
    }
